...
and mixed-effects models to `R`-users. THe main functions allow for fitting
...
@Cremers2018-ta  and @Cremers2021-mm  for a description of the MCMC samplers).
...
Only a couple of R-packages provide methods for circular data analyses and even
less contain functionality for circular regression models. An overview is given
in @pewsey2020recent. The package `circular` [@Agostinelli2017] is a general
purpose package that also contains functionality for fitting frequentist
regression models for circular outcomes based on the von Mises distribution. The
package `circglmbayes` [@Mulder2017] provides a Bayesian regression model for
circular outcomes based on the von Mises distribution. To date `bpnreg` is the
only R-package providing Bayesian regression and mixed-effects models for
...
@Olson2020-al, @Klugkist2018-ag, @Spinks2019-ya] have used `bpnreg` in their
analyses.
...
projected normal mixed-effects models for circular outcomes.


For more examples we refer to @Cremers2018-ys. Answers to frequently asked
questions regarding the use of `bpnreg` can be found in the `FAQ` vignette
available on `CRAN`.
